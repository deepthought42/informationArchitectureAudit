...
public interface PageStateRepository extends Neo4jRepository<PageState, Long> {
	...
	@Query("MATCH (p:PageState) WHERE id(p)=$page_state_id MATCH (element:ElementState) WHERE id(element) IN $element_id_list MERGE (p)-[:HAS]->(element) RETURN COUNT(element)")
	public long addAllElements(@Param("page_state_id") long page_state_id, @Param("element_id_list") List<Long> element_id_list);

	@Query("MATCH (p:PageState) WHERE id(p)=$page_state_id MATCH (element:ElementState) WHERE id(element) IN $element_id_list MERGE (p)-[:HAS]->(element) RETURN element")
