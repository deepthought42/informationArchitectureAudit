...
public interface UXIssueMessageRepository extends Neo4jRepository<UXIssueMessage, Long>  {
	...
	@Query("MATCH (uim:UXIssueMessage) MATCH (e:ElementState) WHERE id(uim)=$issue_id AND id(e)=$element_id MERGE (uim)-[r:FOR]->(e) RETURN uim")
	public void addElement(@Param("issue_id") long issue_id, @Param("element_id") long element_id);

	@Query("MATCH (uim:UXIssueMessage) WHERE id(uim)=$issue_id MATCH (e:PageState) WHERE AND id(e)=$page_id MERGE (uim)-[r:FOR]->(e) RETURN uim")
