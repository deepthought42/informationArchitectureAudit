...
import com.looksee.audit.informationArchitecture.models.journeys.SimpleStep;
...
public interface SimpleStepRepository extends Neo4jRepository<SimpleStep, Long> {
	...
	@Query("MATCH (s:SimpleStep) WHERE id(s)=$step_id MATCH (p:PageState) WHERE id(p)=$page_state_id MERGE (s)-[:STARTS_WITH]->(p) RETURN p")
	public PageState addStartPage(@Param("step_id") long id, @Param("page_state_id") long page_state_id);
	
	@Query("MATCH (s:SimpleStep) WHERE id(s)=$step_id MATCH (p:PageState) WHERE id(p)=$page_state_id MERGE (s)-[:ENDS_WITH]->(p) RETURN p")
	public PageState addEndPage(@Param("step_id") long id, @Param("page_state_id") long page_state_id);
	
	@Query("MATCH (s:SimpleStep) WHERE id(s)=$step_id MATCH(p:ElementState) WHERE id(p)=$element_state_id MERGE (s)-[:HAS]->(p) RETURN p")
