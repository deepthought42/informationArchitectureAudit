...
public interface DomainRepository extends Neo4jRepository<Domain, Long> {
	...
	@Query("MATCH (d:Domain) WHERE id(d)=$domain_id MATCH (p:PageState) WHERE id(p)=$page_id MERGE (d)-[:HAS]->(p) RETURN p")
	...
	@Query("MATCH (d:Domain) WHERE id(d) = $domain_id MATCH (audit:AuditRecord{key:$audit_record_key}) MERGE (d)-[:HAS]->(audit) RETURN audit")
	...
	@Query("MATCH (d:Domain) WHERE id(d)=$domain_id MATCH (design:DesignSystem) WHERE id(design)=$design_system_id MERGE (d)-[:USES]->(design) RETURN design")
	...
	@Query("MATCH (d:Domain) WHERE id(d)=$domain_id MATCH (user:TestUser) WHERE id(user)=$test_user_id MERGE (d)-[:HAS_TEST_USER]->(user) RETURN user")
