...
public interface AuditRecordRepository extends Neo4jRepository<AuditRecord, Long> {
	...
	@Query("MATCH (ar:PageAuditRecord) WHERE id(ar)=$audit_record_id MATCH (a:Audit) WHERE id(a)=$audit_id MERGE (ar)-[h:HAS]->(a) RETURN ar")
	public void addAudit(@Param("audit_record_id") long audit_record_id, @Param("audit_id") long audit_id);
	
	@Query("MATCH (dar:DomainAuditRecord) WHERE id(dar)=$domain_audit_record_id MATCH (par:PageAuditRecord{key:$page_audit_key}) MERGE (dar)-[h:HAS]->(par) RETURN dar")
	...
	@Query("MATCH (ar:AuditRecord) WHERE id(ar)=$audit_record_id MATCH (page:PageState) WHERE id(page)=$page_state_id MERGE (ar)-[h:FOR]->(page) RETURN ar")
